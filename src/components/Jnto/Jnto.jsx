...
const items = [
  { status: false, img: welcome },
  { status: false, img: frame3 },
  { status: false, img: frame4 },
  { status: false, img: frame5 },
  { status: false, img: frame6 },
  { status: false, img: frame7 },
  { status: false, img: frame8 },
...
const Jnto = () => {
  ...
  const slides = items.map((item) => {
    return (
      ...
      >
        ...
        {item.status && (
          <a
            className="jnto-cta-btn"
            href="https://www.japan.travel/en/in/goodnessofjapan/?utm_source=google&utm_medium=cpc&utm_campaign=goodness_of_japan_sumo&gclid=Cj0KCQjwgtWDBhDZARIsADEKwgOWQN0rUcRLtAhZqqgFSnNVdIxR3WPlWChG6GdJ6gjfLwl5_Jf8_ikaAp3yEALw_wcB"
            target="_blank"
            rel="noreferrer"
          >
            Click to PLAY
  ...
  useEffect(() => {
    if (document.getElementsByClassName("carousel-item").length === 8) {
      if (
        document.getElementsByClassName("carousel-item")[5].className ===
